...
## [Video Demo](https://www.youtube.com/watch?v=o1TjPNNUZuE)
...
Our team divided the project into three main parts to work efficiently and improve the self-driving car model step by step. Ron completed the earlier setup and initial work, including preparing the basic project structure and collecting the starting data. Hyun Joon focused on improving the data pipeline by tuning preprocessing, adding data augmentation, and building the batching process so the model could train more effectively on cleaner and more diverse inputs. Nada then concentrated on training the neural network further and testing the final model in the simulator, adjusting the model and training settings until the car could successfully follow the track more reliably.
