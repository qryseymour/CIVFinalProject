...
with open(CSV_PATH, mode="r", encoding='utf-8') as file:
    ...
    for index, row in enumerate(csv_reader, start=1):
        ...
        # Center image
        center_filename = os.path.basename(center_path.replace('\\', '/'))
        local_center = os.path.join(DATA_DIR, 'IMG', center_filename)
        image_list.append(local_center)
        wheel_list.append(steering)

        # Left image (car is left of center – need to turn right)
        left_filename = os.path.basename(left_path.replace('\\', '/'))
        local_left = os.path.join(DATA_DIR, 'IMG', left_filename)
        image_list.append(local_left)
        wheel_list.append(steering + STEERING_CORRECTION)

        # Right image (car is right of center – need to turn left)
        right_filename = os.path.basename(right_path.replace('\\', '/'))
        local_right = os.path.join(DATA_DIR, 'IMG', right_filename)
        image_list.append(local_right)
        wheel_list.append(steering - STEERING_CORRECTION)
...
EPOCHS = 30
train_gen = batch_generator(X_train, y_train, batch_size=BATCH_SIZE, training=True, aug_prob=0.8)
